...
public:
	// person_info의 기본 생성자와 swap은 매우 싸고 빠르다 가정한다.
    person_info() {}
...
int main() {
	...
	// boost::move 라이브러리는 C++11에서는 rvalue에 대한 내부 매크로로 확장되고
	// 만약 C++11 이하를 사용한다면 rvalue 처럼 보이게 흉내를 내게 된다.
	// 즉 어떤 경우든 가리지 않고 확장하여 사용이 가능하다.
